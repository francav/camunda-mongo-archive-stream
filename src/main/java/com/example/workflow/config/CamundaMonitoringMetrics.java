...
public class CamundaMonitoringMetrics {
	...
	public Gauge jobExecutionsSuccessful(MeterRegistry registry) {
		...
		return Gauge.builder("job.executions.successful", query::sum).description("Successful job executions")
				.baseUnit(NUMBER_OF_JOBS).register(registry);
	}
	...
	public Gauge jobExecutionsFailed(MeterRegistry registry) {
		...
		return Gauge.builder("job.executions.failed", query::sum).description("Failed job executions")
				.baseUnit(NUMBER_OF_JOBS).register(registry);
	}
	...
	public Gauge jobExecutionsRejected(MeterRegistry registry) {
		...
		return Gauge.builder("job.executions.rejected", query::sum)
				.description("Rejected jobs due to saturated execution resources").baseUnit(NUMBER_OF_JOBS)
				.register(registry);
	}
	...
	public Gauge jobAcquisitionsAttempted(MeterRegistry registry) {
		...
		return Gauge.builder("job.acquisitions.attempted", query::sum).description("Performed job acquisition cycles")
				.baseUnit(NUMBER_OF_ACQUISITION_CYCLES).register(registry);
	}
	...
	public Gauge jobAcquisitionsSuccessful(MeterRegistry registry) {
		...
		return Gauge.builder("job.acquisitions.successful", query::sum).description("Successful job acquisitions")
				.baseUnit(NUMBER_OF_JOBS).register(registry);
	}
	...
	public Gauge jobAcquistionsFailed(MeterRegistry registry) {
		...
		return Gauge.builder("job.acquisitions.failed", query::sum).description("Failed job acquisitions")
				.baseUnit(NUMBER_OF_JOBS).register(registry);
	}
	...
	public Gauge jobLocksExclusive(MeterRegistry registry) {
		...
		return Gauge.builder("job.locks.exclusive", query::sum)
				...
				.baseUnit(NUMBER_OF_EXCLUSIVE_JOBS).register(registry);
	}
	...
	public Gauge dueJobsInDB(MeterRegistry registry) {
		...
		return Gauge.builder("jobs.due", jobQuery::count).description("Jobs from async continuation that are due")
				.register(registry);
	}
}
